...
<xsl:template match="/">
	...
	<table style="font-family: Helvetica, Arial, sans-serif; font-size: 14px; width: 700px; color: #363636; margin-left: auto; margin-right: auto;">
		<tbody>
			...
			<tr>
				<td><h2 style="font-size: 18px; font-weight: bold; line-hight: 1.2em; margin-top: 0; margin-bottom: 20px;"><xsl:value-of select="data/newsletter-f-alt/entry/title" /></h2></td>
			</tr>
		</tbody>
	</table>
...
<xsl:template match="linked-posty/entry">
	...
	<xsl:if test="cover">
		<img src="{$workspace}{cover/@path}/{cover/filename}" alt="{title-pl}" style="width: 100%; margin-bottom: 20px" />
	</xsl:if>

	<xsl:if test="series-pl">
		<h2 style="display: inline-block; padding: 3px 9px 6px; font-size: 14px; line-height: normal; margin-top: 0; margin-bottom: 20px; border: 2px solid;"><xsl:value-of select="series-pl"/></h2>
	</xsl:if>

	<h1 style="margin-top: 0; margin-bottom: 10px; font-size: 28px; line-height: 1.2;"><a href="{$root}/pl/post/{title-pl/@handle}" target="_blank" style="color: inherit; text-decoration: none;"><xsl:value-of select="title-pl"/></a></h1>

	<xsl:if test="subtitle-pl">
		<h2 style="font-size: 18px; font-weight: bold; margin-top: 0; margin-bottom: 20px;"><xsl:value-of select="subtitle-pl"/></h2>
	</xsl:if>

	<xsl:if test="date-place-pl">
		<p style="margin-top: 0; margin-bottom: 20px; font-size: 16px; font-weight: bold;"><xsl:value-of select="date-place-pl"/></p>
	</xsl:if>

	<xsl:copy-of select="lead-pl/node()"/>

	<p style="margin-top: 20px; margin-bottom: 40px;"><strong><a style="width: 160px; display: inline-block; margin-right: 20px; text-decoration: none; color: #343434;" href="{$root}/pl/post/{title-pl/@handle}">Czytaj dalej</a><!--<a style=" text-decoration: none; color: #343434;" href="{$root}/en/post/{title-pl/@handle}">Read more</a>--></strong></p>
...
<xsl:template match="linked-zatrudnieni/entry">
	...
	<xsl:if test="image">
		<img src="{$workspace}{image/@path}/{image/filename}" alt="{name-pl}" style="width: 100%; margin-bottom: 20px" />
	</xsl:if>


	<h1 style="margin-top: 0; margin-bottom: 10px; font-size: 28px; line-height: 1.2;"><a href="{$root}/pl/people/{name-pl/@handle}" target="_blank" style="color: inherit; text-decoration: none;"><xsl:value-of select="name-pl"/></a></h1>

	<xsl:if test="job-pl">
		<h2 style="font-size: 18px; font-weight: bold; margin-top: 0; margin-bottom: 20px;"><xsl:value-of select="job-pl"/></h2>
	</xsl:if>
	...
	<p style="margin-top: 20px; margin-bottom: 40px;"><strong><a style="width: 160px; display: inline-block; margin-right: 20px; text-decoration: none; color: #343434;" href="{$root}/pl/people/{name-pl/@handle}">Czytaj dalej</a><a style=" text-decoration: none; color: #343434;" href="{$root}/en/people/{name-pl/@handle}">Read more</a></strong></p>
