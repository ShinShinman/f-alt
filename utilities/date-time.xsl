...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="format-month">
		...
		<xsl:param name="month-word">
			...
			<xsl:choose>
				<xsl:when test="$month = 01">Stycznia</xsl:when>
				<xsl:when test="$month = 02">Lutego</xsl:when>
				<xsl:when test="$month = 03">Marca</xsl:when>
				<xsl:when test="$month = 04">Kwietnia</xsl:when>
				<xsl:when test="$month = 05">Maja</xsl:when>
				<xsl:when test="$month = 06">Czerwca</xsl:when>
				<xsl:when test="$month = 07">Lipca</xsl:when>
				<xsl:when test="$month = 08">Sierpnia</xsl:when>
				<xsl:when test="$month = 09">Września</xsl:when>
				<xsl:when test="$month = 10">Października</xsl:when>
				<xsl:when test="$month = 11">Listopada</xsl:when>
				<xsl:when test="$month = 12">Grudnia</xsl:when>
			</xsl:choose>
